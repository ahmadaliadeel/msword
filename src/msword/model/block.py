...
schema-version handling, and the paragraph-iter protocol. Unit-6 only needs the
shape of those interfaces so its block types can register at import and round-trip
through the registry. Field names and method signatures here are chosen to match
what unit-5 will land, so the unit-5 file simply replaces this stub without
forcing changes in unit-6.
"""
...
class ParagraphSpec:
    ...
    A block's `iter_paragraphs` yields these. The composer doesn't care which
    block produced a paragraph, only its style ref and runs.
    """
...
class BlockRegistry:
    ...
    Block subclasses register at import time so JSON round-trips can resolve
    nested children polymorphically.
    """
...
class ParagraphBlock(Block):
    """Stub paragraph block used by unit-6 tests for block-nesting roundtrips.

    # stub: replaced by unit-5

    Unit-5 ships the full ParagraphBlock with style refs and richer attrs.
    """
